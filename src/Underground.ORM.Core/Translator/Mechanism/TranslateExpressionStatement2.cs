...
namespace Underground.ORM.Core.Translator
{
    ...
    {
        private SyntaxBase TranslateExpressionStatement(string csFileContent,
                                                         List<SyntaxNodeOrToken> descendants,
                                                         SyntaxBase mysqlSyntaxOut)
        {
            SyntaxBase mysqlExpression = new();
        ...
        private void ReplacePlusOperatorStringToCommaConcat(SyntaxBase expression,
                                                            SyntaxBase mysqlSyntaxOut)
        {
            for (int i = 0; i < expression.Count; i++)
            {
                SyntaxTokenBase item = expression[i];
        ...
        private void InsertCoalesceToAllVariablesReference(SyntaxBase expression,
                                                           SyntaxBase mysqlSyntaxOut)
        {
            for (int i = 0; i < expression.Count; i++)
            {
                ...
                if (item.Is.String)
                {
                    expression.AppendAt(i, new CoalesceFunctionToken());
        ...
        private (SyntaxTokenBase Function, SyntaxBase? Alias)
        ...
        private SyntaxBase BuildRightCastFunction(ElevatorCastExpression lastElevator)
        {
            SyntaxBase castFunction = new CloseParenthesisToken();
